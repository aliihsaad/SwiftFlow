...
export async function loadContentIntelligenceSignals(workspaceId: string): Promise<ContentIntelligenceSignals> {
  ...
  const [{ data: brandProfile }, { data: socialAccounts }, { data: posts }] = await Promise.all([
    ...
    admin.from("social_accounts").select("id, platform, metadata").eq("workspace_id", workspaceId),
  ...
  if (postIds.length > 0) {
    const { data } = await admin.from("published_posts").select("*").in("post_id", postIds).limit(100)
