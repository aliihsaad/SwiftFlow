...
serve(async (req) => {
    ...
    try {
        ...
        console.log('[generate-message-reply] Reply generated length:', reply.length)
        return new Response(JSON.stringify({ reply }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })

    } catch (error: any) {
        console.error('[generate-message-reply] Error:', redactSensitiveLogValue(error?.message || error))
