...
export function AssistantContextReceiptView({ receipt }: AssistantContextReceiptProps) {
  ...
  return (
    <div className="mt-2 flex max-w-full flex-wrap items-center gap-1.5 text-[11px] text-white/38">
      <span className="inline-flex items-center gap-1 rounded-full border border-white/8 bg-white/5 px-2 py-1">
      ...
      {receipt.warnings.slice(0, 2).map((warning) => (
        <span key={warning} className="inline-flex items-center gap-1 rounded-full border border-amber-400/15 bg-amber-400/10 px-2 py-1 text-amber-100/70">
